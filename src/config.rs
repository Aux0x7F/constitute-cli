...
use crate::cli::Cli;
use crate::transport::TransportOpenOptions;
...
impl AppContext {
    pub fn from_cli(cli: &Cli) -> Result<Self> {
        Ok(Self {
            ...
            test_data_dir: cli.fixture_dir.clone(),
            json: cli.json,
            passphrase: std::env::var("CONSTITUTE_CLI_PASSPHRASE").ok(),
        })
    }

    pub fn uses_test_data_transport(&self) -> bool {
        self.test_data_dir.is_some()
    }

    pub fn test_data_path(&self) -> Option<PathBuf> {
        self.test_data_dir.clone()
    }
    ...
    pub fn transport_options(
        ...
        TransportOpenOptions {
            fixture_dir: self.test_data_dir.clone(),
            profile,
            device_secret,
        }
    }
